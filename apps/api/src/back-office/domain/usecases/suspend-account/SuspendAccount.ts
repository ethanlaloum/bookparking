...
/**
 * Suspend un compte. La suspension bloque la connexion — c'est `SignIn` qui
 * la fait respecter ; sans cela elle ne serait qu'une mention décorative. Les
 * annonces et locations déjà en cours ne sont pas touchées.
 *
 ...
 * être administrateur.
 */
export class SuspendAccount implements UseCase<
  Props,
  Promise<
    Either.Either<
      void,
...
> {
  ...
  > {
    try {
      ...
      const done = await this.backOfficeRepository.setAccountSuspension(
        props.targetId,
        props.actedAt,
      );
      ...
      await this.backOfficeRepository.recordAction({
        ...
        kind: AdminActionKind.SUSPEND_ACCOUNT,
